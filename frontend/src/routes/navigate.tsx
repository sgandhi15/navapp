...
import { useRoute, formatDistance, formatDuration, formatETA } from '../hooks/useRoute'
...
function NavigatePage() {
  ...
  const { location, isWatching, startWatching, stopWatching } = useGeolocation()
  const { saveAddress } = useAddresses()
  const { data: routeData, isLoading: routeLoading, refetch: refetchRoute } = useRoute(
    location?.lat ?? null,
    location?.lng ?? null,
    destLat,
    destLng
        ...
        <div className="bg-slate-800/95 backdrop-blur-sm rounded-t-3xl shadow-2xl p-6 pb-8">
          ...
          <div className="flex items-start gap-3 mb-6">
            ...
            <div className="flex-1 min-w-0">
              <p className="text-slate-400 text-xs uppercase tracking-wide mb-1">Destination</p>
          ...
          {routeLoading ? (
            ...
            <div className="grid grid-cols-3 gap-4">
              ...
              <div className="bg-slate-700/50 rounded-xl p-4 text-center">
                <p className="text-slate-400 text-xs uppercase tracking-wide mb-1">Distance</p>
              ...
              <div className="bg-emerald-500/20 rounded-xl p-4 text-center">
                <p className="text-emerald-400 text-xs uppercase tracking-wide mb-1">ETA</p>
                <p className="text-2xl font-bold text-emerald-400">{formatETA(routeData.duration)}</p>
              </div>
            </div>
          ) : (
            <div className="text-center py-4 text-slate-400">
              {!location ? 'Waiting for location...' : 'Unable to calculate route'}
            </div>
          )}
